...
resolve :: (Functor m, Monad m) => Program -> [Goal] -> m [Unifier]
resolve program goals = map cleanup <$> runReaderT (resolve' 1 [] goals []) (createDB (builtins ++ program) ["false","fail"])   -- NOTE Is it a good idea to "hardcode" the builtins like this?
  where
      ...
      resolve' :: (Functor m, MonadReader Database m )
      ...
      getBranches :: (MonadReader Database m)
      ...
      choose :: (Monad m, MonadReader Database m)
      ...
      backtrack :: (Monad m, MonadReader Database m)
                   => Int -> Stack -> m [ Unifier ]
